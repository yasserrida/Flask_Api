...
def upload_file_and_get_base64(request):
    ...
    if allowed_file(file.filename):
        file_destination = os.path.join('storage/temp', str(datetime.today().timestamp()) + secure_filename(file.filename))
        file.save(file_destination)
        with open(file_destination, "rb") as image_file:
...
def detect_mime_type(base64):
    ...
    for s in signatures:
        if (base64.find(s) != -1): return signatures[s]
    return False
...
def pdf_from_base64_to_images_base64(base_64):
    ...
    for i, image in enumerate(images_from_path):
        ...
        with open(fname, "rb") as image_file:
            paths_results.append(base64.b64encode(image_file.read()).decode('utf-8'))
...
def ocr(base_64=None):
    results = []
    mime_type = detect_mime_type(base_64)
    if (mime_type == "application/pdf"):
        imgs_64 = pdf_from_base64_to_images_base64(base_64)
        for img_64 in imgs_64:
            results.append(pytesseract.image_to_string(img_64))
    elif (mime_type): 
        results.append(pytesseract.image_to_string(base_64))
    return results
